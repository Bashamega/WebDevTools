...
export default function Search({ isDarkMode }: SearchProps) {
  const [searchValue, setSearchValue] = useState(""); // State to store the value of search input
  const [searchResults, setSearchResults] = useState<filteredData[]>([]); // State to store the filtered search results

  //const [showDropdown, setShowDropdown] = useState(false); // State to control the visibility of the dropdown
  const [showDropdown, setShowDropdown] = useState<boolean>(false); // State to control the visibility of the dropdown
  const inputRef = useRef<HTMLInputElement>(null); // Reference to the input element
  const dropdownRef = useRef<HTMLUListElement>(null); // Reference to the dropdown list element
