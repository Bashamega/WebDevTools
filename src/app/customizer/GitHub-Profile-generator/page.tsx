...
export default function ButtonCustomizer() {
  ...
  const SkillIcon = ({ skill }: { skill: keyof typeof skillsIcons }) => {
    const icon =
      typeof skill === "string" ? skillsIcons[skill.toLowerCase()] : "";
    return icon ? (
      <Image
  ...
  const generateMarkdown = (formValues: FormData, skills: string[]): string => {
    let markdown = "";
...
// function setIsDarkMode(arg0: (prevMode: boolean) => boolean) {
