...
const ColorPicker = () => {
  ...
  interface ImageLoadEvent extends ProgressEvent<FileReader> {
  ...
  interface DrawImageOnCanvas {
    (ctx: CanvasRenderingContext2D, image: string): void;
  }

  const drawImageOnCanvas: DrawImageOnCanvas = (ctx, image) => {
    const img = new Image();
    img.onload = () => {
      if (canvasRef.current) {
        canvasRef.current.width = img.width;
