...
describe("CardForm Component", () => {
  ...
  test("renders the initial fields from Init.jsx", () => {
    ...
    const initialFieldNames = fieldNameInputs.map(
  ...
  test('generates JSON data and downloads file when "Export" button is clicked', async () => {
    ...
    const filename = (saveAs as unknown as jest.Mock).mock.calls[0][1];
  ...
  test("handles bigint values by converting them to strings", async () => {
    ...
    const blob = (saveAs as unknown as jest.Mock).mock.calls[0][0];
  ...
  test("generates correct output in the preview after changing field type", async () => {
    ...
    const jsonContent = JSON.parse(previewOutput.textContent as string);
  ...
  test("updates the preview output after closing and changing field type", async () => {
    ...
    const updatedJsonContent = JSON.parse(
      updatedPreviewOutput.textContent as string,
    );
  ...
  test("updates preview after adding a field and setting its type", async () => {
    ...
    jsonContent.forEach((entry: { [key: string]: any }) => {
    ...
    const unchangedJsonContent = JSON.parse(
      unchangedPreviewOutput.textContent as string,
    );
    ...
    unchangedJsonContent.forEach((entry: { [key: string]: any }) => {
    ...
    updatedJsonContent.forEach((entry: { [key: string]: any }) => {
