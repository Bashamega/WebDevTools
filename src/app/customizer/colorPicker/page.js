...
export default function colorPicker() {
  ...
  const handleHex = () => navigator.clipboard.writeText(hexColor);
  const handleRGB = () => navigator.clipboard.writeText(toRGB(hexColor));
  ...
  return (
    <main>
      ...
      <div className="grid grid-rows-2 grid-cols-1 gap-y-16 sm:grid-cols-2 sm:grid-rows-1 sm:gap-x-8 h-[100vh] w-[100vw] overflow-hidden">
        ...
        <div className="flex flex-col jusitfy-center items-center mb-8 sm:pt-[270px] xl:pr-36 xl:pt-[300px]">
          <h1 className="font-bold text-[5rem] sm:text-[4rem] lg:text-7xl xl:text-8xl pb-3">{hexColor}</h1>
          <h1 className="font-bold text-4xl sm:text-4xl lg:text-4xl xl:text-5xl pb-8">{RGBColor}</h1>
