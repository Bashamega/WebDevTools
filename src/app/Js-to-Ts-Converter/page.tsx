...
const Converter: React.FC<ConverterProps> = () => {
  ...
  const convertJsToTs = (jsCode: string): string => {
    let tsCode = jsCode
      // Convert object declarations with inferred types for common data types
      .replace(
        /const\s+(\w+)\s*=\s*{([^}]+)}/g,
        (match: string, varName: string, props: string) => {
      ...
      .replace(
        /const\s+(\w+)\s*=\s*\[([^;]+)\]/g,
        (match: string, varName: string, elements: string) => {
  ...
  interface InferArrayType {
    (elements: string): string;
  }

  const inferArrayType: InferArrayType = (elements) => {
