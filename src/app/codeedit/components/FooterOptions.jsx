            ...
            {Popup ? (
                <section className="absolute top-0 left-0 flex justify-center items-center w-screen h-screen backdrop-blur-lg">
                    <div className="p-5 rounded-lg shadow bg-gray-700 w-1/2 max-h-1/2">
                        <CloseSharp onClick={() => { setPop(false) }} className="cursor-pointer" />
                        <select onChange={(e) => setLocalLang(e.target.value)} className="bg-slate-600 rounded-lg p-2 hover:bg-slate-700 cursor-pointer text-center w-full flex items-center justify-center">
                        ...
                        <button onClick={chooseLang} className="mt-2 w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Apply
                        </button>
                    </div>
                </section>
