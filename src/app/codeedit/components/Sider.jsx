            ...
            <nav className="h-[10vh] bg-slate-900 shadow-2xl">
                <div className="h-full w-full flex justify-center items-center flex-col">
                    ...
                    <div className="flex justify-center items-center bg-black h-full w-full">
                        <NoteAdd className="cursor-pointer hover:text-slate-500" onClick={() => setNewFile(true)} />
                    </div>
                </div>
            </nav>
            <section className="p-1 h-[90vh]">
                {newFile ? <form onSubmit={handleSubmit}>
                    <input type="text" value={fileName} onChange={(e) => setFile(e.target.value)} placeholder="File Name" required minLength={1} className="w-full h-full bg-slate-900 p-2 rounded-lg text-white" />
                </form> : null}
                <div className="mt-2 gap-3 grid grid-cols-1">
                    {files.length > 0 ? files.map((file, index) => (
                        <button key={index} className="flex justify-center items-center bg-slate-900 p-2 rounded-lg text-white" onClick={() => activateFile(file)}>
                            {file.name}
                        </button>
