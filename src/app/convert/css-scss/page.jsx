...
export default function CSS_TO_SCSS() {
  ...
  const [mode, setMode] = useState({ from: "css", to: "scss" });
  ...
  const handleConvert = () => {
    if(mode.to === "scss") return setResultSnippet(cssToScss(snippet))
    else return setResultSnippet(scssToCss(snippet))
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center flex-col items-center w-full">
        ...
        <div
          className={`mr-4 p-5 mt-9 w-4/5 break-words py-6 md:p-6 border rounded-lg shadow ${isDarkMode ? "bg-gray-900 text-white" : "bg-white text-black"} flex items-center gap-4`}
        >
          <SnippetBox
            from={mode.from}
