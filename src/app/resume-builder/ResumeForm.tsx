...
const ResumeForm: React.FC<ResumeFormProps> = ({
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files[0]) {
      ...
      reader.onloadend = () => {
        // Safely update formData by using the callback form of setState
        setFormData((prevState) => {
          ...
          return updatedFormData; // Update the state with the new value
        });
      };
  ...
  const handleLinksChange = (e: LinksChangeEvent) => {
    const { name, value } = e.target;
  ...
  //       onFormChange({ ...formData, image: reader.result as string });
  //     };
  //     reader.readAsDataURL(file);
  //   }
  // };
  ...
  <h3 className="text-2xl font-semibold">Work Experience</h3>
  {formData.workExperience.map((job, index) => (
    <div key={job.title + job.company + index} className="grid grid-cols-3 gap-6 items-start mb-4">
      <div>
        <h4
          className={`text-base font-medium my-2 ${isDarkMode ? "text-slate-300" : "text-slate-500"}`}
        >
      ...
      <div className="space-y-4">
        ...
        {formData.workExperience.map((job, index) => (
          <div
            key={job.id || index}
            className="grid grid-cols-3 gap-6 items-start mb-4"
          >
            {/* Work Title */}
            <div>
              <h4
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-row items-start justify-between my-4 flex-wrap w-full">
          {formData.skills.map((skillCategory, index) => (
            <div key={skillCategory.category + index} className="mb-4">
              ...
              {skillCategory.skills.map((s, id) => (
                <input
                  ...
                  key={skillCategory.category + id} // Ensure unique key for input
                />
              ))}
            </div>
          ))}
