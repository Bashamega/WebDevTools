...
interface BasicModalProps {
  ...
  handleParametersURL: (labels: string[], isSelected: boolean) => void;
}
...
export default function BasicModal({
  ...
  useEffect(() => {
    setLabelsList((prevLabelsList) => {
      return [...new Set([...prevLabelsList, ...selectedLabels])];
    });
  }, [open, selectedLabels]); // Include both `open` and `selectedLabels` as dependencies
  ...
  const handleChange = (inputLabel: string, event: HandleChangeEvent) => {
  ...
  const handleFilterInput = () => {
    if (filterInput.length <= 1 || labelsList.includes(filterInput)) {
      ...
      setLabelsList((prevLabelsList) => [...prevLabelsList, filterInput]);
      setFilterInput("");
    }
  };
