...
export default function GhFinder() {
  ...
  const [data, setData] = useState<IssueWithPRs[]>([]);
  const [selectedLabels, setSelectedLabels] = useState<string[]>([]);
  const [selectedLabelsData, setSelectedLabelsData] = useState<Label[]>([]);
  const [filteredIssues, setFilteredIssues] = useState<IssueWithPRs[]>([]);
  ...
  const fetchPRsForIssue = useCallback(
    async (issue: Issue): Promise<string[]> => {
      ...
      try {
        ...
        return linkedPRs
          .map((pr) => pr.source?.issue?.pull_request?.html_url)
  ...
  const addPRsInfoToIssues = useCallback(
    async (issues: Issue[]): Promise<IssueWithPRs[]> => {
  ...
  useEffect(() => {
    ...
    const updateData = (newData: Issue[] | IssueWithPRs[]): void => {
      const now: number = new Date().getTime();
      localStorage.setItem(cacheKey, JSON.stringify(newData));
      localStorage.setItem(cacheExpirationKey, now.toString());

      setData(newData as IssueWithPRs[]);
    };
    ...
    const fetchDataAndSet = async () => {
      ...
      let filteredData =
        selected === 1
          ? result
            ? (result as Issue[]).filter(
                (item) => !item.node_id.includes("PR_"),
              )
            : []
          : result
            ? (result as FetchResult).items
            : [];
    ...
    if (
      !(
        cachedData &&
        cacheTimestamp &&
        now - Number(cacheTimestamp) < cacheExpirationTime
      )
    ) {
      fetchDataAndSet();
    } else {
      setData(JSON.parse(cachedData) as IssueWithPRs[]);
    }
  }, [
    fetchData,
    selected,
  ...
  interface HandleParametersURL {
    (parameters: string[], isAddingNew: boolean): void;
  }

  const handleParametersURL: HandleParametersURL = (
    parameters,
    isAddingNew,
  ) => {
    if (isAddingNew) {
  ...
  function handleSelectedLabel(inputLabel: Label) {
